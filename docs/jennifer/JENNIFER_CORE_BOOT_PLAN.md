...
- 개선 제안 → 사용자 승인 → 반영
- 승인 없는 코어 변경 금지
- 금융/자산 관련 고위험 액션은 `주인님 + 제니` 공동관리(dual control) 기본 적용

### C5. Finance Copilot Safety Core

- 금융 노드는 기본적으로 **제안자(signal)** 역할
- 실행 노드는 격리된 권한으로 동작하며 승인 범위를 벗어난 주문 금지
- 신규진입/비중확대/손절무시 등 고위험 액션은 2중 승인 필수
- 자동매매는 제한 위임(한도/유효시간/허용종목/킬스위치) 기반으로만 허용
