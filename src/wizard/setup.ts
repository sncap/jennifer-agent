...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Jennifer is still in beta. Expect sharp edges.",
      "By default, Jennifer is a personal agent: one trusted operator boundary.",
      ...
      "",
      "Run regularly:",
      "jennifer security audit --deep",
      "jennifer security audit --fix",
      "",
...
export async function runSetupWizard(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("jennifer doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("jennifer doctor")}`,
        `Inspect: ${formatCliCommand("jennifer plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("jennifer configure")}.`;
