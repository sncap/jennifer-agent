    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["jennifer nodes status", "List known nodes with live status."],
          ["jennifer nodes pairing pending", "Show pending node pairing requests."],
          ['jennifer nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["jennifer nodes camera snap --node <id>", "Capture a photo from a node camera."],
