...
const EXAMPLES = [
  ["jennifer models --help", "Show detailed help for the models command."],
  [
    "jennifer channels login --verbose",
    "Link personal WhatsApp Web and show QR + connection logs.",
  ],
  [
    'jennifer message send --target +15555550123 --message "Hi" --json',
    "Send via your web session and print JSON result.",
  ],
  ["jennifer gateway --port 18789", "Run the WebSocket Gateway locally."],
  ["jennifer --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["jennifer gateway --force", "Kill anything bound to the default gateway port, then start it."],
  ["jennifer gateway ...", "Gateway control via WebSocket."],
  [
    'jennifer agent --to +15555550123 --message "Run summary" --deliver',
    "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply.",
  ],
  [
    'jennifer message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  ...
  if (
    ...
    console.log(
      commit ? `Jennifer ${ctx.programVersion} (${commit})` : `Jennifer ${ctx.programVersion}`,
    );
    process.exit(0);
  }
